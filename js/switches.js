function switchDevice(cur){
	...
	if(typeof(idx)=='string' && idx.substr(0,1)=='s'){
		$.ajax({
			url: '/json.htm?type=command&param=switchscene&idx='+idx.replace('s','')+'&switchcmd='+doStatus+'&level=0&passcode=&jsoncallback=?',
		...
		$.ajax({
			url: '/json.htm?type=command&param=switchlight&idx='+idx+'&switchcmd='+doStatus+'&level=0&passcode=&jsoncallback=?',
			type: 'GET',async: false,contentType: "application/json",dataType: 'jsonp',
			success:function(data) {
				getDevices(true);
			}
...
function switchThermostat(setpoint,cur){
	...
	if(typeof(req)!=='undefined') req.abort();
	req = $.ajax({
		url: '/json.htm?type=command&param=setsetpoint&idx='+idx+'&setpoint='+setpoint+'&jsoncallback=?',
		type: 'GET',async: false,contentType: "application/json",dataType: 'jsonp',
		success:function(data) {
...
function switchBlinds(idx,action){
	...
	if(typeof(req)!=='undefined') req.abort();
	$.ajax({
		url: '/json.htm?type=command&param=switchlight&idx='+idx+'&switchcmd='+action+'&level=0&passcode=&jsoncallback=?',
		type: 'GET',async: false,contentType: "application/json",dataType: 'jsonp',
		success:function(data) {
			getDevices(true);
		}
...
function slideDevice(idx,status){
	...
	slide = $.ajax({
		url: '/json.htm?type=command&param=switchlight&idx='+idx+'&switchcmd=Set%20Level&level='+status+'&jsoncallback=?',
		type: 'GET',async: false,contentType: "application/json",dataType: 'jsonp',
		success:function(data) {
			getDevices(true);
		}
...
function controlLogitech(idx,action){
	$.ajax({
		url: '/json.htm?type=command&param=lmsmediacommand&idx='+idx+'&action='+action+'&jsoncallback=?',
