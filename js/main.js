...
function getMoonInfo(image){
   req = $.getJSONP({
      url: "/json.htm?type=command&param=getuservariable&idx="+settings['idx_moonpicture']+"&jsoncallback=?",
...
function getDevices(override){
	...
	if(!sliding || override){
		...
		if(settings['domoticz_ip']=='http://192.168.1.10:1407'){
			...
			req = $.getJSONP({
			url: '/json.htm?type=devices&filter=all&used=true&order=Name&jsoncallback=?',
			type: 'GET',async: true,contentType: "application/json",dataType: 'jsonp',
								...
								if(typeof(device['SubType'])!=='undefined' && device['SubType'] in blocktypes['SubType']){
									...
									if(device['SubType']=='removeRGBW' || device['SubType']=='removeRGBWW'){
										...
										$(".rgbw").on("dragstop.spectrum",function(e, color) {
											...
											sliding=true;
											var url = '/json.htm?type=command&param=setcolbrightnessvalue&idx='+curidx+'&hue='+hue.h+'&brightness='+hue.b+'&iswhite='+bIsWhite;
											$.ajax({
